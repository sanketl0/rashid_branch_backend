...
from .Salescustomer_model import SalesCustomer
from .Invoice_model import Invoice
from .Pr_model import PR
...
class Multiple_Invoice_Details(BaseDateid):
    mul_inv_id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    customer_id = models.ForeignKey(SalesCustomer, on_delete=models.SET_NULL, null=True)
    company_id = models.ForeignKey(Company, on_delete=models.SET_NULL, null=True)
    company_yearid = models.ForeignKey(Company_Year, on_delete=models.SET_NULL, null=True)
    invoice_id = models.ForeignKey(Invoice, on_delete=models.SET_NULL, null=True, related_name='payments_made')
    branch_id = models.ForeignKey(Branch, on_delete=models.SET_NULL, null=True)
    amount = models.DecimalField(decimal_places=2, max_digits=15, blank=True, null=True)
    amount_due = models.DecimalField(decimal_places=2, max_digits=15, blank=True, null=True)
    pr_id = models.ForeignKey(PR, on_delete=models.SET_NULL, null=True, related_name='payments_made')
    insert_amount = models.DecimalField(decimal_places=2, max_digits=15, blank=True, null=True)
